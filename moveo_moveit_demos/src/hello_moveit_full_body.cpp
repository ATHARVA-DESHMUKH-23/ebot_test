
#include <geometry_msgs/msg/pose_stamped.hpp>
#include <memory>
#include <rclcpp/rclcpp.hpp>
#include <moveit/move_group_interface/move_group_interface.hpp>
#include <thread>
#include <rclcpp/executors/single_threaded_executor.hpp>
#include <tf2_geometry_msgs/tf2_geometry_msgs.hpp>
#include <tf2/LinearMath/Matrix3x3.h>
#include <tf2_ros/transform_broadcaster.h>
#include <geometry_msgs/msg/transform_stamped.hpp>

/**
 * @brief The main function that starts our program.
 *
 * This function sets up our ROS 2 environment and prepares it for robot control.
 *
 * @param argc The number of input arguments our program receives.
 * @param argv The list of input arguments our program receives.
 * @return int A number indicating if our program finished successfully (0) or not.
 */
int main(int argc, char * argv[])
{
  // Start up ROS 2
  rclcpp::init(argc, argv);

  // Creates a node named "hello_moveit_full_body_full_body". The node is set up to automatically
  // handle any settings (parameters) we might want to change later without editing the code.
  auto const node = std::make_shared<rclcpp::Node>(
    "hello_moveit_full_body_full_body",
    rclcpp::NodeOptions().automatically_declare_parameters_from_overrides(true)
  );

  auto tf_broadcaster = std::make_shared<tf2_ros::TransformBroadcaster>(node);
  
  // 🔥 ADD THIS BLOCK (RIGHT HERE)
  rclcpp::executors::SingleThreadedExecutor executor;
  executor.add_node(node);
  std::thread([&executor]() { executor.spin(); }).detach();

  // Creates a "logger" that we can use to print out information or error messages
  // as our program runs.
  auto const logger = rclcpp::get_logger("hello_moveit_full_body_full_body");

  // Create the MoveIt MoveGroup Interfaces
  // These interfaces are used to plan and execute movements, set target poses,
  // and perform other motion-related tasks for each respective part of the robot.
  // The use of auto allows the compiler to automatically deduce the type of variable.
  // Source: https://github.com/moveit/moveit2/blob/main/moveit_ros/planning_interface/move_group_interface/include/moveit/move_group_interface/move_group_interface.h
  using moveit::planning_interface::MoveGroupInterface;
  auto arm_group_interface = MoveGroupInterface(node, "arm");
  using moveit::planning_interface::MoveGroupInterface;

  auto full_body_group_interface =
      MoveGroupInterface(node, "mobile_base_manipulator");

  // Get robot model
  auto robot_model =
      full_body_group_interface.getRobotModel();

  // Get joint model group
  auto* jmg =
      robot_model->getJointModelGroup(
          "mobile_base_manipulator");

  // Print all variables in the group
  RCLCPP_INFO(logger, "Variables in mobile_base_manipulator:");

  for (const auto& v : jmg->getVariableNames())
  {
      RCLCPP_INFO(logger, "%s", v.c_str());
  }

  auto state = full_body_group_interface.getCurrentState(1.0);

  std::vector<double> vals;
  state->copyJointGroupPositions(jmg, vals);

  for(size_t i=0;i<vals.size();i++)
  {
      RCLCPP_INFO(logger,"%s = %.3f",
          jmg->getVariableNames()[i].c_str(),
          vals[i]);
  }

  std::map<std::string, double> target;

//JOINT TARGET

  // target["planar_joint/x"] = 0.5;
  // target["planar_joint/y"] = 0.0;
  // target["planar_joint/theta"] = 0.0;

  // target["Joint_1"] = 0.0;
  // target["Joint_2"] = 0.0;
  // target["Joint_3"] = 0.0;
  // target["Joint_4"] = 0.0;
  // target["Joint_5"] = 0.0;

  // full_body_group_interface.setJointValueTarget(target);
  // Specify a planning pipeline to be used for further planning

//joint target end


  RCLCPP_INFO(
    logger,
    "Planning frame: %s",
    full_body_group_interface.getPlanningFrame().c_str()
  );

  full_body_group_interface.setPlanningPipelineId("ompl");

  // Specify a planner to be used for further planning
  full_body_group_interface.setPlannerId("RRTConnectkConfigDefault");

  // Specify the maximum amount of time in seconds to use when planning
  full_body_group_interface.setStartStateToCurrentState();
  full_body_group_interface.setPlanningTime(5.0);
  full_body_group_interface.setNumPlanningAttempts(50);

  // Set a scaling factor for optionally reducing the maximum joint velocity. Allowed values are in (0,1].
  full_body_group_interface.setMaxVelocityScalingFactor(1.0);

  //  Set a scaling factor for optionally reducing the maximum joint acceleration. Allowed values are in (0,1].
  full_body_group_interface.setMaxAccelerationScalingFactor(1.0);

  // Display helpful logging messages on the terminal
  RCLCPP_INFO(logger, "Planning pipeline: %s", full_body_group_interface.getPlanningPipelineId().c_str());
  RCLCPP_INFO(logger, "Planner ID: %s", full_body_group_interface.getPlannerId().c_str());
  RCLCPP_INFO(logger, "Planning time: %.2f", full_body_group_interface.getPlanningTime());
  while (!full_body_group_interface.getCurrentState(1.0)) {
    RCLCPP_WARN(logger, "Waiting for current robot state...");
  }
  // auto pose = arm_group_interface.getCurrentPose();
  // RCLCPP_INFO(logger, "Current: x=%.3f y=%.3f z=%.3f pose_x=%.3f pose_y=%.3f pose_z=%.3f pose_w=%.3f",
  //   pose.pose.position.x,
  //   pose.pose.position.y,
  //   pose.pose.position.z,
  //   pose.pose.orientation.x,
  //   pose.pose.orientation.y,
  //   pose.pose.orientation.z,
  //   pose.pose.orientation.w);

  auto pose = full_body_group_interface.getCurrentPose();

  // Convert quaternion → RPY
  tf2::Quaternion q(
    pose.pose.orientation.x,
    pose.pose.orientation.y,
    pose.pose.orientation.z,
    pose.pose.orientation.w
  );

  double roll, pitch, yaw;
  tf2::Matrix3x3(q).getRPY(roll, pitch, yaw);

  RCLCPP_INFO(logger,
    "Position: x=%.3f y=%.3f z=%.3f | RPY: roll=%.3f pitch=%.3f yaw=%.3f",
    pose.pose.position.x,
    pose.pose.position.y,
    pose.pose.position.z,
    roll, pitch, yaw
  );
  auto current_pose = full_body_group_interface.getCurrentPose();
  double x, y, z;

  node->get_parameter("target_x", x);
  node->get_parameter("target_y", y);
  node->get_parameter("target_z", z);

  // Use the current pose as a starting point and modify it to create the target pose
  geometry_msgs::msg::Pose target_pose = current_pose.pose;

  target_pose.position.x = x;
  target_pose.position.y = y;
  target_pose.position.z = z;
  // target_pose.orientation.x = -0.455;
  // target_pose.orientation.y = 0.456;
  // target_pose.orientation.z = 0.541;
  // target_pose.orientation.w = 0.541;
  // target_pose.orientation.x = -0.631;
  // target_pose.orientation.y = 0.764;
  // target_pose.orientation.z = -0.041;
  // target_pose.orientation.w = -0.126;
  target_pose.orientation.x = -0.5;
  target_pose.orientation.y = 0.5;
  target_pose.orientation.z = -0.5;
  target_pose.orientation.w = 0.5;
  RCLCPP_INFO(logger, "Target received: x=%.3f y=%.3f z=%.3f", x, y, z);
  // full_body_group_interface.setPositionTarget(
  //   x, y, z
  // );
  
  // Publish the target pose as a TF transform for visualization in RViz
  geometry_msgs::msg::TransformStamped t;

  t.header.stamp = node->now();
  t.header.frame_id = "odom";   // same frame you are planning in
  t.child_frame_id = "target_pose";

  t.transform.translation.x = target_pose.position.x;
  t.transform.translation.y = target_pose.position.y;
  t.transform.translation.z = target_pose.position.z;

  t.transform.rotation = target_pose.orientation;

  // publish once (or loop if needed)
  tf_broadcaster->sendTransform(t);

  full_body_group_interface.setGoalOrientationTolerance(0.54159); // ~31 degrees
  full_body_group_interface.setPoseTarget(target_pose);
//   double roll_target = 0.0;
//   double pitch_target = 0.0;   // try 90 deg
//   double yaw_target = 0.0;

//   tf2::Quaternion target_q;
//   target_q.setRPY(roll_target, pitch_target, yaw_target);
//   target_q.normalize();

//   target_pose.orientation = tf2::toMsg(target_q);
// x=0.019 y=0.001 z=0.638

  // Create a plan to that target pose
  // This will give us two things:
  // 1. Whether the planning was successful (stored in 'success')
  // 2. The actual motion plan (stored in 'plan')
  auto const [success, plan] = [&full_body_group_interface] {
    moveit::planning_interface::MoveGroupInterface::Plan msg;
    auto const ok = static_cast<bool>(full_body_group_interface.plan(msg));
    return std::make_pair(ok, msg);
  }();

  // Try to execute the movement plan if it was created successfully
  // If the plan wasn't successful, report an error
  // Execute the plan
  if (success)
  {
    RCLCPP_INFO(logger, "Planning succeeded!");
    // full_body_group_interface.execute(plan);
  }
    else
  {
    RCLCPP_ERROR(logger, "Planning failed!");
  }

  // Shut down ROS 2 cleanly when we're done
  rclcpp::shutdown();
  return 0;
}