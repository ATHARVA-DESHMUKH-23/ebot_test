...
int main(int argc, char * argv[])
{
  ...
  RCLCPP_INFO(logger, "Max velocity scaling factor: %.2f", arm_group_interface.getMaxVelocityScalingFactor());
  ...
  auto moveit_visual_tools =
      moveit_visual_tools::MoveItVisualTools{ node, "moveo_base_link", rviz_visual_tools::RVIZ_MARKER_TOPIC,
  ...
  //   x, y, z
  // );
  arm_group_interface.setGoalOrientationTolerance(0.54159); // ~31 degrees
  ...
  auto const collision_object = [frame_id = arm_group_interface.getPlanningFrame(), &node, &logger] {
    ...
    box_pose.position.z = 0.65;  // meters in z-direction
