...
private:
  ...
  void targetCB(const geometry_msgs::msg::Point::SharedPtr msg)
  {
    ...
    RCLCPP_INFO(get_logger(),
      "🎯 Target = [%.3f %.3f %.3f]", target_.x(), target_.y(), target_.z());
  }

  /* ---------------- control loop ---------------- */
  void controlLoop()
  {
    if (!joints_ok_ || !target_ok_)
      return;
    ...
    Eigen::Vector3d err = target_ - x_cur;

    if (err.norm() < 0.005) return;  // ⭐ anti-jitter

    /* ---- Jacobian ---- */
    KDL::Jacobian J_kdl(chain_.getNrOfJoints());
    jac_solver_->JntToJac(q, J_kdl);

    Eigen::MatrixXd J(3, chain_.getNrOfJoints());
    for (int r = 0; r < 3; r++)
    ...
    Eigen::VectorXd qdot =
      J.transpose() *
      (J * J.transpose() +
       lambda * lambda * Eigen::Matrix3d::Identity()).inverse()
      * (2.0 * err);

    /* ---- integrate ---- */
    Eigen::VectorXd q_cmd = Eigen::VectorXd::Zero(chain_.getNrOfJoints());
