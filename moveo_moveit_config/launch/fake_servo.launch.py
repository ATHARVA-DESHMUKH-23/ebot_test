...
def generate_launch_description():
    ...
    declare_use_fake_base_cmd = DeclareLaunchArgument(
        name='use_fake_base',
        default_value='false',
        description='Enable mobile base in manual control (fake servo)'
    )
    ...
    def configure_setup(context):
        ...
        use_fake_base_bool = use_fake_base.lower() == 'true'
        ...
        fake_servo_node = Node(
            package="moveo_moveit_demos",
            executable="fake_servo",
            output="screen",
            parameters=[
                moveit_config.to_dict(),
                {'use_sim_time': use_sim_time},
        ...
        return [start_move_group_node_cmd, fake_servo_node, start_rviz_node_cmd, exit_event_handler]
    ...
    ld.add_action(declare_use_fake_base_cmd)
