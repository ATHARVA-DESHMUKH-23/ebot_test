...
bool StretchKinematicsPlugin::timedOut(const rclcpp::Time& start_time, double duration) const
{
  return ((node_->now() - start_time).seconds() >= duration);
}

bool StretchKinematicsPlugin::getPositionIK(const geometry_msgs::msg::Pose& ik_pose,
...
bool StretchKinematicsPlugin::getPositionFK(const std::vector<std::string>& link_names,
...
const std::vector<std::string>& StretchKinematicsPlugin::getLinkNames() const
{
...
// register StretchKinematicsPlugin as a KinematicsBase implementation
#include <class_loader/class_loader.hpp>
CLASS_LOADER_REGISTER_CLASS(stretch_kinematics_plugin::StretchKinematicsPlugin, kinematics::KinematicsBase)
