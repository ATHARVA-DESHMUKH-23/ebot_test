...
Task createTask(const rclcpp::Node::SharedPtr& node) {
  ...
  // Add a stage to move to the "Upright" position
  {
    auto stage = std::make_unique<stages::MoveTo>("move to Upright", std::make_shared<solvers::JointInterpolationPlanner>());
    stage->setGroup(group);
    stage->setGoal("Upright");
    t.add(std::move(stage));
    RCLCPP_INFO(node->get_logger(), "Added stage: Move to 'Upright' position");
  }
