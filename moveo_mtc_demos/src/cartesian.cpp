...
Task createTask(const rclcpp::Node::SharedPtr& node) {
  ...
  const std::string arm_with_hand = "arm_with_hand";
  ...
  // Set the initial state of the robot
  {
    ...
    state.setToDefaultValues(state.getJointModelGroup(arm_with_hand), "Ready");
  ...
  // Stage 1: Move 0.05 meters in the positive x direction relative to the base_link frame
  {
    ...
    stage->setIKFrame("Link_5");

    // Create a Vector3Stamped message to specify the direction of movement
    geometry_msgs::msg::Vector3Stamped direction;

    // Set the frame of reference for this movement (the "ebot_base_link" frame)
    direction.header.frame_id = "ebot_base_link";
  ...
  // Stage 3: Rotate 18 degrees around the z-axis of the base_link frame
  {
    ...
    twist.header.frame_id = "ebot_base_link";
  ...
  // Stage 4: Move specific joints by specified angles
  {
    ...
    std::map<std::string, double> offsets = {
      { "Joint_1", M_PI / 12. },  // Rotate this joint by 15 degrees (pi/12 radians)
      { "Joint_3", -M_PI / 12. }   // Rotate this joint by -15 degrees
    };
