...
int main(int argc, char** argv) {
    ...
    assert(t.getRobotModel()->getName() == "ebot");
    ...
    robot_state.setToDefaultValues(robot_state.getJointModelGroup("arm"), "upright");
    std::cout << "Initialized robot state in planning scene to default values and 'upright' position" << std::endl;
    ...
    // Strategy 1: Minimize path length
    {
      auto connect{ std::make_unique<stages::Connect>(
           "path length", stages::Connect::GroupPlannerVector{ { "arm_with_hand", pipeline } }) };
    ...
    {
      auto connect{ std::make_unique<stages::Connect>(
           "trajectory duration", stages::Connect::GroupPlannerVector{ { "arm_with_hand", pipeline } }) };
    ...
    {
      auto connect{ std::make_unique<stages::Connect>(
           "eef motion", stages::Connect::GroupPlannerVector{ { "arm_with_hand", pipeline } }) };
      connect->setCostTerm(std::make_unique<cost::LinkMotion>("Link_5")); // Distance traveled by the link in meters
      alternatives->add(std::move(connect));
    ...
    {
      auto connect{ std::make_unique<stages::Connect>(
           "elbow motion", stages::Connect::GroupPlannerVector{ { "arm_with_hand", pipeline } }) };
      connect->setCostTerm(std::make_unique<cost::LinkMotion>("Link_3"));
      alternatives->add(std::move(connect));
    ...
    goal_scene->getCurrentStateNonConst().setToDefaultValues(robot_state.getJointModelGroup("arm_with_hand"), "Ready");
