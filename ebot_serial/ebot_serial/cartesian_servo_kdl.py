...
class CartesianServo(Node):
    ...
    def __init__(self):
        ...
        # ---- KDL ----
        self.kdl_chain = self.build_kdl_chain()
        self.kdl_jac_solver = kdl.ChainJntToJacSolver(self.kdl_chain)

        self.get_logger().info(
            f"KDL initialized with {self.kdl_chain.getNrOfJoints()} joints"
        )
    ...
    def build_kdl_chain(self):
        ...
        chain.addSegment(
            kdl.Segment(
                ...
                kdl.Frame(kdl.Vector(0.0, 0.0, l1))
            )
        )
        ...
        chain.addSegment(
            kdl.Segment(
                ...
                kdl.Frame(kdl.Vector(0.0, 0.0, l2))
            )
        )
        ...
        chain.addSegment(
            kdl.Segment(
                ...
                kdl.Frame(kdl.Vector(0.0, 0.0, l3))
            )
        )
        ...
        chain.addSegment(
            kdl.Segment(
                "tool0",
                kdl.Joint(),
                kdl.Frame(kdl.Vector(0.0, 0.0, l4))
            )
        )

        return chain

    def numpy_to_kdl_joints(self, q):
    ...
    def control_loop(self):
        ...
        # ---- KDL Jacobian ----
        # 🔴 DEAD BAND — DO NOTHING IF NO CARTESIAN COMMAND
        if np.linalg.norm(self.xdot) < 1e-3:
            return
        q_kdl = self.numpy_to_kdl_joints(self.q)

        jac_kdl = kdl.Jacobian(self.kdl_chain.getNrOfJoints())
        self.kdl_jac_solver.JntToJac(q_kdl, jac_kdl)

        J_full = self.kdl_jacobian_to_numpy(jac_kdl)
        ...
        J = np.vstack([
            ...
            J_full[1],  # vy
            J_full[2],  # vz
            J_full[4],  # wy (rotation about Y for wrist)
        ])
